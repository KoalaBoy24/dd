...
async function freeTradeUnlimited(username, coins, packs1) {
    while(true){
        let tradeRef;
        try {
...
function verifyWallet(wallet, coins, cards, packs2, verb, walletOwner) {
...
bot.on("withdraw", async (interaction, coins, cards, packs3)=>{
    ...
    try {
        ...
        const wallet = await db.getWallet(userId);
        await withdraw(interaction, userId, username, coins, verifyWallet(wallet, coins, cards, packs3, "withdraw", "your"));
    } finally{
        db.endTransaction(userId);
    }
});
bot.on("pay", async (interaction, otherUserId, coins, cards, packs4)=>{
    ...
    try {
        await interaction.acknowledge();
        const wallet = await db.getWallet(userId);
        const walletVerification = verifyWallet(wallet, coins, cards, packs4, "pay", "your");
        if (!walletVerification.success) {
...
bot.on("admin-pay", async (interaction, otherUserId, coins, cards, packs5)=>{
    const coinsError = verifyCoins(coins, 0, Number.MAX_SAFE_INTEGER, "pay");
    if (coinsError) {
        interaction.createMessage(coinsError);
        return;
...
bot.on("trade", async (interaction, otherUserId, givingCoins, givingCards, givingPacks, receivingCoins, receivingCards, receivingPacks)=>{
    ...
    try {
        ...
        for (const card3 of otherFinalCards){
            transactions.push(db.addCards(userId, card3.id, card3.amount));
            transactions.push(db.addCards(otherUserId, card3.id, -card3.amount));
        }
        for (const pack of myFinalPacks){
            transactions.push(db.addPacks(otherUserId, pack.id, pack.amount));
            transactions.push(db.addPacks(userId, pack.id, -pack.amount));
        }
        for (const pack3 of otherFinalPacks){
            transactions.push(db.addPacks(userId, pack3.id, pack3.amount));
            transactions.push(db.addPacks(otherUserId, pack3.id, -pack3.amount));
        }
        await Promise.all(transactions);
    } finally{
        db.endTransaction(userId);
